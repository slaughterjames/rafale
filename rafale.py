...
def VirusTotal():
    ...
    newlogentry = 'VirusTotal Data has been downloaded to a file here: <a href=\"' + LOG.rafaleroot + 'samples/' + AP.sampleid + '/' + 'VirusTotalReport.html' + '\"> VirusTotal Report </a>' 
    LOG.WriteLog(AP.sampleid, newlogentry)
    newlogentry = ''

    newlogentry = 'The live VirusTotal Data can be found here: <a href=\"'  + target + '\"> VirusTotal Report </a>'
    AP.live_virus_total = 'The live VirusTotal Data can be found here: '  + target 
    LOG.WriteLog(AP.sampleid, newlogentry)
    newlogentry = ''    
...
def Strings():
    ...
    subproc = subprocess.Popen('strings '+ LOG.temp + AP.filename, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    for strings_data in subproc.stdout.readlines():
         AP.strings_output_data += strings_data
         if  (AP.debug == True):
             print strings_data    

    try:
        FI.WriteLogFile(filename, AP.strings_output_data)
        newlogentry = 'Strings file has been generated to file here: <a href=\"' + LOG.rafaleroot + 'samples/' + AP.sampleid + '/' + 'Strings.txt' + '\"> Strings Output </a>'
        LOG.WriteLog(AP.sampleid, newlogentry)
        newlogentry = ''
    except:
        newlogentry = 'Unable to perform strings against uploaded file'
        LOG.WriteLog(AP.sampleid, newlogentry)
        newlogentry = ''
...
def PEFile():
    ...
    for section in PEX.sections:
       newlogentry = 'Section Name: <strong>' + section.Name + '</strong> Virtual Address: <strong>' + str(hex(section.VirtualAddress)) + '</strong> Virtual Size: <strong>' + str(hex(section.Misc_VirtualSize)) + '</strong> Raw Data Size: <strong>' +  str(section.SizeOfRawData) + '</strong>'
       LOG.WriteLog(AP.sampleid, newlogentry)
       newlogentry = ''
    ...
    try:
        for entry in PEX.DIRECTORY_ENTRY_IMPORT:
            ...
            for imp in entry.imports:
                ...
                newlogentry = '\t' +  ' <strong>' + str(hex(imp.address)) +  ' ' + str(imp.name) + '</strong>'
                LOG.WriteLog(AP.sampleid, newlogentry)
                newlogentry = ''
    except:
    ...
    try:
        for exp in PEX.DIRECTORY_ENTRY_EXPORT.symbols:
            ...
            newlogentry = '<strong>' + hex(pe.OPTIONAL_HEADER.ImageBase + exp.address), exp.name, exp.ordinal + '</strong>'
            LOG.WriteLog(AP.sampleid, newlogentry)
            newlogentry = ''
    except:
...
def MoveAndZip():
    ...
    if (AP.debug==True):
        print 'zip -j ' + LOG.temp + AP.sampleid + '.zip ' + LOG.temp + AP.filename 

    #Add a password if chosen otherwise, ZIP up the file
    if (AP.passwordYorN == True):
        subproc = subprocess.Popen('zip -j -P ' + AP.password + ' ' + LOG.temp + AP.sampleid + '.zip ' + LOG.temp + AP.filename, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    else:
        subproc = subprocess.Popen('zip -j ' + LOG.temp + AP.sampleid + '.zip ' + LOG.temp + AP.filename, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

    newlogentry = 'Sample has been saved to file here: <a href=\"' + LOG.samplesroot + 'repo/' + AP.sampleid + '.zip' + '\"> Download Sample </a>'
    LOG.WriteLog(AP.sampleid, newlogentry)
    newlogentry = ''
    ...
    newlogentry = 'Original sample has now been removed from the repo directory.'
    LOG.WriteLog(AP.sampleid, newlogentry)
    newlogentry = ''   
...
if __name__ == '__main__':
        ...
        LOG.temp = LOG.samplesrepo.strip()
        ...
        if (LOG.signaturedb != ''):
            SIG = peutils.SignatureDatabase(LOG.signaturedb)
